...
export default function EventDetailPanel({ eventId, onClose }) {
  ...
  const addToMyDay = usePlanStore((s) => s.addToMyDay);
  const myDayEvents = usePlanStore((s) => s.myDayEvents);
  const removeFromMyDay = usePlanStore((s) => s.removeFromMyDay);
      ...
      {!loading && event && (
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="space-y-2 pt-2">
            ...
            {isPlanOpen && (() => {
              const inDay = myDayEvents.some((e) => e.id === eventId);
              return inDay ? (
                <button
                  onClick={() => removeFromMyDay(eventId)}
                  className="w-full text-center text-sm font-medium py-2 rounded-lg border border-green-300 text-green-600 bg-green-50 transition-colors"
                >
                  ✓ Added to My Day
                </button>
              ) : (
                <button
                  onClick={() => addToMyDay(event)}
